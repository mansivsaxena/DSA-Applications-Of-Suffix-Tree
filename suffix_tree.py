...
class Suffix(object):
    ...
    def explicit(self):
        #A suffix is explicit if it ends on a node. first_char_index is made greater than last_char_index to show this.
...
class SuffixTree(object):
   #suffix tree using ukkonen's alg for construction
    ...
    def _canonize_suffix(self, suffix):
        #This canonizes the suffix by moving along the suffix string until it is explicit or there are no more matched nodes
    ...
    def find_all_occurences(self, substring):
        # Finds all occurences of a substring in the suffix tree by DFS traversal from the node at which substring ends
        # Returns list of indices at which match was found. If none, return a list containing -1.
        if not substring:
            return [-1]
        if self.case_insensitive:
            substring = substring.lower()
        ...
        if(self.nodes[curr_node].suffix_node != -1 and flag == 0):
            for i in self.edges.values():
                if(i.source_node_index == curr_node):
                    # Do a dfs traversal to find all the leaves 
